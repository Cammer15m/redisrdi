...
# Get all dashboards
dashboards=$(docker exec re-n1 curl -X GET \
  http://admin:${PASSWORD}@grafana:3000/api/search \
  -H 'Content-Type: application/json' \
  -d '{"query": ""}' | jq -r '.[] | .uid')

# Delete each dashboard
for dashboard in $dashboards; do
  docker exec re-n1 curl -X DELETE \
    http://admin:${PASSWORD}@grafana:3000/api/dashboards/uid/$dashboard \
    -H 'Content-Type: application/json'
done
